...
const AddCar = () => {
  ...
  useEffect(() => {
    if (editCarId && cars.length > 0) {
      const existingCar = cars.find((c) => c._id === editCarId);
      if (existingCar) {
        setIsEditMode(true);
        setCar({
          ...
          pricePerDay: existingCar.pricePerDay || 0,
        ...
        setExistingImage(existingCar.image);
      }
    }
  }, [editCarId, cars]);
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        formData.append("carId", editCarId);
      }
      
      const { data } = isEditMode
        ? await axios.put("/api/owner/update-car", formData)
        : await axios.post("/api/owner/add-car", formData);
  ...
  return (
    <div className="px-4 py-10 md:px-10 flex-1">
      ...
      <form
        ...
        className="flex flex-col gap-5 text-slate-400 text-sm mt-6 max-w-xl"
      >
        {/* Car Image */}
        <div className="flex items-center gap-2 w-full">
          <label htmlFor="car-image">
            <img
              src={image ? URL.createObjectURL(image) : (existingImage ? existingImage : assets.upload_icon)}
              alt=""
          ...
          <p className="text-sm text-slate-400">Upload a picture of your car</p>
        </div>
        {/* Car brand and model */}
      ...
      text-white rounded-md font-medium w-max cursor-pointer"
        >
          ...
          {isLoading ? "Saving..." : (isEditMode ? "Update Car" : "List Your Car")}
        </button>
      </form>
    </div>
  );
